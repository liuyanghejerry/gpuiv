...
describeNative("markdown-editor cross-block selection / clipboard / anchors", () => {
  ...
  it("clears cross-block and native selection when clicking a paragraph gap", async () => {
    const { app } = selApp("# first heading\n\nsecond paragraph\n")
    await app.settle()
    const blocks = app.renderer.findByType("textarea")
    const [x, y] = app.renderer.getInputTextPosition(blocks[0].id, 2)
    const [endX, endY] = app.renderer.getInputTextPosition(blocks[1].id, 6)
    app.renderer.nativeSimulateMouseDown(x, y + 2, 0)
    app.renderer.nativeSimulateMouseMove(endX, endY + 2, 0)
    await app.settle()
    app.renderer.nativeSimulateMouseUp(endX, endY + 2, 0)
    await app.settle()
    expect(app.renderer.getInputDecorations(blocks[1].id).length).toBeGreaterThan(0)
    const bounds = app.renderer.getElementBounds(blocks[1].id)!
    app.renderer.nativeSimulateMouseDown(bounds.x + 10, bounds.y - 4, 0)
    app.renderer.nativeSimulateMouseUp(bounds.x + 10, bounds.y - 4, 0)
    await app.settle()
    expect(app.renderer.getInputDecorations(blocks[1].id)).toHaveLength(0)
    app.renderer.writeClipboardText("unchanged")
    app.renderer.nativeSimulateKeystrokes(blocks[0].id, "cmd-c")
    await app.settle()
    expect(app.renderer.readClipboardText()).toBe("unchanged")
    app.unmount()
  })
