//! GPUI accessibility props from Vue custom_props.
//!
...
/// Apply `role` and `aria-*` from Vue, plus a default role when the app
...
/// VoiceOver Press. GPUI auto-adds Click only for `.on_click()`. GPUIV
