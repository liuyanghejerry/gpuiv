...
Headless controls in `@gpuiv/vue` (`select`, `combobox`, `tooltip`, and any
...
mounted `SelectItem` children (a provide/inject registry filled on mount). Do
not walk `child.type`. Do not require `items` for the menu to work.
...
**Always** add a `.changeset/*.md` file after a user-facing fix or feature. Do this before you consider the work done. Never skip it. Never edit `packages/*/CHANGELOG.md` — they are generated by `changeset version` from the `.changeset/` folder (the root `CHANGELOG.md` is a pointer to them). Never bump `package.json` version by hand.
