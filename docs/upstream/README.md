...
| Infinite-chat example | `50e08b9` | pending | bidirectional pagination example on `<virtual-list>` + `PublicInstance` typed refs; its dependency, the pinning/anchoring topic above, is now synced (PR #24), so this is unblocked. Its browser-serving half (`de1c74a`) went to the declined wasm row |
