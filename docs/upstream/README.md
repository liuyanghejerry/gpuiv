...
| Infinite-chat example | `50e08b9` | synced | Vue port on feat/infinite-chat (PR pending): host `<virtual-list>` with retained children + `onVisibleRange`; the React `PublicInstance` ref maps to our already-exported `HostNode`; `SafeMdxContent` gained an `onLinkClick` render ctx in chat.tsx; `string-dedent` inlined as a local helper (no new dep); React's `flushSync` ordering is native's queued `scrollToItem`, tests settle twice after `scrollToItem` because visibleRange dispatches at flush end |
