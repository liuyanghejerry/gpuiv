...
| Chat demo upgrade (threads, replies, history, overlays, inspector) | `136d4f5` `c98b414` (search-overlay + composer-focus halves) | pending | per-thread transcripts, demo send/retry replies, retry, nav-stack history arrows, search overlay (+ its `pointerEvents` layering fix), settings overlay, inspector panel, projectOnly filter, composer focusTick (~860 lines of their React chat.tsx); we implement the same behaviors in Vue idioms over our diverged flagship — in flight as PR-B, will land as `diverged` |
