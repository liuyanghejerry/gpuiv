...
Inventoried range: `367ef48..d85a31e` (2026-09-16). Earlier history is in
[Sync log](#sync-log) below.

| Topic | Upstream commits | Status | Notes |
|---|---|---|---|
| Wasm / browser rendering, website | `75a1fe6` `92082a3` `7d6e1f9` `711945f` `7f6732a` `7a3e356` `bbcea1f` `291b92d` `a906cf9` `f52fa54` `de1c74a` `4194d9b` `3bb1ac6` `5388016` | declined | [wasm-web-rendering.md](./wasm-web-rendering.md) — `de1c74a` (serve infinite-chat at `/infinite`) and `4194d9b` (compact website layout) extend the same topic; `3bb1ac6`'s website-tooling half joins it (its README keyboard reference rides with the Tab-key port); `5388016` is the homepage hero (Arizona Flare font + Holocron shader video) |
| CI / release plumbing | `0b257d0` `301b834` `ac0426c` `322993e` `e68f2ec` `dfb83f3` `a24b4a4` `89b6799` | declined | one-target-per-OS matrix and archive shipping serve their matrix, not ours; our CI publishes both packages; `322993e` `e68f2ec` `dfb83f3` are their 0.6.0 release + bun-publish npmrc fixes; `a24b4a4` is their 0.7.0 release, `89b6799` their 0.8.0 release (our releases go through changesets + CI) |
...
| Canvas element transform (zed submodule, not a React-binding topic) | — | researched | [canvas-transform.md](./canvas-transform.md) — `PolychromeSprite` has no `transformation` field; rotation has no path today. Pinch half shipped in PR #63; waits on the fork change |
| Canvas atlas sub-rect writes (zed submodule, not a React-binding topic) | — | researched | [canvas-atlas-partial-write.md](./canvas-atlas-partial-write.md) — `PlatformAtlas` has no sub-rect update; PR #66 ships the 256×256 tile-grid workaround; canvas memory / GPU-raster follow-ups recorded there |
| `cursor: none` / custom image cursors (zed submodule, not a React-binding topic) | — | researched | [cursor-none-custom.md](./cursor-none-custom.md) — `CursorStyle` has no hidden/image variants; 29 keywords → 21 variants shipped in PR #65; fork needed for the rest |
...
| Select follows Base UI | `846de94` (Select half) | synced | PR #75 — root `items` optional label lookup for `SelectValue`, provide/inject registry of mounted `SelectItem`s (mount hooks, not per render) replaces `collectItems`, root `disabled` gates nav/select, late-mounted selected item becomes the highlight via a sync watcher (React's `useLayoutEffect` on `itemsVersion`), `textValue` dropped, controlled `value`/`open` read through getters; fork-only fix: `FloatingLayer` `inheritAttrs: false` stops fallthrough attrs (`tabIndex`/`autoFocus`) from duplicating onto the `anchored` root and racing the content div for popup focus; the `useFocusTrap` hunk stays declined (see that row) |
...
| wlr-layer-shell window kind | `bbaa202` `e47bd1c` | synced | PR #86 — napi `WindowOptions.layerShell` + `appId` mapping to `WindowKind::LayerShell` (Linux/Wayland only, cfg-gated; zed already had `gpui::layer_shell` at `81c99f8`, no submodule bump), layer surfaces open at the origin in `init_threaded`, README docs + changeset; Linux-gated tests compile on the CI matrix but no Linux test runner executes them; `index.d.ts` regenerated by our build (upstream hand-edited theirs) |
| Deferred focus queue | `ca1a01d` | synced | PR #89 — `GpuixView::request_focus` shared by the macOS napi, `UiCommand::FocusElement`, Wasm, and test-renderer entry points: immediate focus when the handle exists (clearing any older pending request), else `pending_focus_element` consumed by the next `sync_focus_handles` after the autoFocus pass; latest-request-wins, explicit beats `autoFocus`, never-focusable targets drop on the next render as before. Fork deltas: the test renderer routes through `request_focus` too (upstream left it on the old inline path), `TestApp.flushMutations()` exposes the tree-without-paint window, `deferred-focus.test.tsx` pins both rules |
| `asChild` for Select/Combobox items | `ef9b670` | pending | GPUI has no click bubbling, so a filled custom row covers the item's hit target; React's `asChild` makes the custom row the item host itself. Needs a Vue-side API design first (slot-owned item host vs documented fill-on-the-item + `pointerEvents: none` inside) — discuss before porting |
| Chat picker row-fill fix (`c98b414` picker half) | `c98b414` (picker half) | synced | PR #90 — fill/hover/cursor moved onto the `SelectItem` style (`menuItemStyle`), `MenuRow` root + `Icon` svgs `pointerEvents: 'none'`, item testIds; the bug was reproduced on our tree first (row click never reached the item). Regression: chat row-click test + two Select pins (transparent row lets the pick through; filled row covers it — platform truth); README ModelPicker example corrected. Upstream's later `asChild` migration stays its own pending topic |
| Chat demo upgrade (threads, replies, history, overlays, inspector) | `136d4f5` `c98b414` (search-overlay + composer-focus halves) | diverged | PR #91 — same behavior set, own Vue implementation over our diverged chat.tsx: per-thread `turns` state with only the active thread mounted (keyed Transcript), New Task + draft-derived titles, send→demoReply, retry, nav-stack arrows, search/settings `OverlayCard` (pointer-transparent dim + card-level `onMouseDownOutside`), Inspector, projectOnly, focusTick composer refocus via template ref + `focusElement`; c1 keeps the `expandTurns` perf corpus, reply copy GPUIV-ified. Revisit only if the demo's contract with the library changes (e.g. it starts depending on host events we lack) |
...
our ported subprocess tests already — `events.test.tsx` and
`runtime-error-overlay.test.tsx` have used `fileURLToPath` from the start.

**Last inventoried upstream head:** `d85a31e` (2026-09-16)
...
| #75 | Select follows Base UI: Root `items` as `SelectValue` label lookup, mounted-`SelectItem` provide/inject registry (mount hooks, not per render), root-disabled gating, late-mounted selected item highlight, controlled-prop getters for `useControllableState`, `FloatingLayer` `inheritAttrs: false` focus-race fix (fork-only), six GPU-backed Select tests — upstream `846de94` (Select half) |
| #86 | Layer-shell window kind: napi `WindowOptions.layerShell` + `appId` → `WindowKind::LayerShell` (Linux/Wayland; zed already had `gpui::layer_shell`, no bump), layer surfaces open at the origin, README docs; ledger round `18e695e..0fac5c9` also declines `5388016` (website hero) into the web topic — upstream `bbaa202` |
| #89 | Deferred focus queue: `GpuixView::request_focus` shared by all four focus entry points (macOS napi, `UiCommand::FocusElement`, Wasm, test renderer — upstream left the test renderer on the old path), `pending_focus_element` consumed in `sync_focus_handles` after the autoFocus pass, `TestApp.flushMutations()` + `deferred-focus.test.tsx`; ledger round `0fac5c9..d85a31e` adds pending rows (`asChild`, chat-example threads/polish) and declined rows (browser boot fixes, README rewrites, idle-hello gitignore, 0.8.0 release) — upstream `ca1a01d` |
| #90 | Chat picker row-fill fix: fill/hover/cursor onto `SelectItem` style, `MenuRow`/`Icon` pointer-transparent, item testIds; chat row-click regression + two Select filled-row pins (pattern + platform truth), README ModelPicker example corrected — upstream `c98b414` (picker half) |
| #91 | Chat demo upgrade as a Vue-idiom rebuild (diverged): per-thread transcripts (threads as state, keyed Transcript), New Task + draft titles, send→demoReply + retry, nav-stack history, search/settings overlays with the pointer-transparent-dim + card `onMouseDownOutside` pattern, Inspector, projectOnly, focusTick composer refocus (deferred focus from #89); six new behavior tests, tail-scroll assertions settle twice — upstream `136d4f5` `c98b414` (non-picker halves) |
